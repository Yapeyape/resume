...
const experiences = [
  ...
  ['2017 — 2024', 'Founder & Business Owner', 'Suojaväri Oy — house painting & façade maintenance · Central Finland · 10+ seasonal employees, ~€300k turnover', 'Founded and grew a façade painting business, managing everything from client acquisition to project delivery. Stepped away in 2024 to focus on university studies and freelance projects.'],
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#242729] text-[#f3f6f8]">
      ...
      <main className="resume-sheet">
        ...
        <section className="resume-section" id="creative"><SectionTitle icon={Palette}>Bonus: Content creation</SectionTitle><p className="section-intro">A glimpse at the photography, editing and visual content I create.</p><div className="creative-grid"><article><h3>Video editing</h3><p>Filming and editing video content for digital channels and client content.</p><div className="video-stage"><video ref={videoRef} className="creative-video" autoPlay muted playsInline onEnded={() => setVideoIndex((index) => (index + 1) % videos.length)}><source src={media(videos[videoIndex])} type="video/mp4" /></video></div><div className="dots">{videos.map((name, index) => <button className={index === videoIndex ? 'active' : ''} key={name} onClick={() => setVideoIndex(index)} aria-label={name} />)}</div></article><article><h3>Still images and editing</h3><p>Still images and edited visual work from personal and creative projects.</p><div className="still-stage">{showingProfiles ? <div className="profile-bubbles">{profiles.map((name) => <img key={name} src={media(name)} alt={name} />)}</div> : <img src={media(stills[stillIndex])} alt={`Photo ${stillIndex + 1}`} />}</div></article></div></section>
