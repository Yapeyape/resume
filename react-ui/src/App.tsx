...
const media = (name: string) => `${import.meta.env.BASE_URL}resume-media/${encodeURIComponent(name).replace(/%2F/g, '/')}`
...
const experiences = [
  ...
  ['2017 — 2024', 'Founder & Business Owner', 'Suojaväri Oy — house painting & façade maintenance · Central Finland', 'Founded and grew a façade painting business, managing everything from client acquisition to project delivery. Left the business in good hands to focus on university studies.'],
...
function App() {
  ...
  const navItems = useMemo(() => [['about', 'About'], ['experience', 'Experience'], ['projects', 'Projects'], ['creative', 'Creative'], ['education', 'Education'], ['contact', 'Contact']], [])

  return (
    <BackgroundPaths>
      <nav className="resume-nav" aria-label="Resume sections">{navItems.map(([id, label]) => <a key={id} href={`#${id}`}>{label}</a>)}</nav>
      <main className="resume-sheet">
        <header className="resume-hero" id="about">
          ...
          <div className="about-copy"><p>I&apos;m a 27-year-old recent graduate with an M.Sc. in Information Systems from the University of Jyväskylä, where my thesis examined how software developers experience generative AI tools. Before and during my studies, I founded and ran a house painting business that grew to 10+ seasonal employees and nearly €300k in turnover. That experience taught me project management, client acquisition, and leading a team from the ground up.</p><p>I combine that hands-on business background with technical skills and practical experience in web development, visual content editing and creation, alongside earlier experience in sales, customer service, and customer acquisition. I&apos;m drawn to work at the intersection of digital services, data, and visual design - where technical thinking meets execution.</p><p>Outside of work, I spend time with family and friends, stay active through sports, love traveling, and enjoy photography and video editing when I get the chance.</p></div>
        </header>

        <section className="resume-section capabilities"><h2>Capabilities</h2>{[['Web Development', 'AI-assisted development · WordPress · Website maintenance'], ['Creative', 'Content creation · Graphic design · UX/UI design · Photoshop · Illustrator · Photography · Video editing'], ['Business', 'Project management · Client acquisition · Marketing · Team leadership · Sales']].map(([title, text]) => <div className="cap-row" key={title}><h3>{title}</h3><p>{text}</p></div>)}</section>
        <section className="resume-section" id="experience"><h2>Experience</h2>{experiences.map(([when, title, org, text]) => <article className="job" key={title}><p className="job-when">{when}</p><div><h3>{title}</h3><p className="job-org">{org}</p><p>{text}</p></div></article>)}</section>
        <section className="resume-section" id="education"><h2>Education</h2><article className="job"><p className="job-when">2021 — 2026</p><div><h3>B.Sc. &amp; M.Sc., Information Systems</h3><p className="job-org">University of Jyväskylä · Kauppatieteiden maisteri, pääaineena tietojärjestelmätiede</p><p>Information systems, data, digitalization, marketing and finance.</p><div className="theses"><a href="https://jyx.jyu.fi/jyx/Record/jyx_123456789_92372"><span>Bachelor&apos;s thesis · Grade 5</span><strong>AI Utilization in Consumer Customer Journey</strong><em>Check thesis →</em></a><a href="https://jyx.jyu.fi/jyx/Record/jyx_123456789_112063"><span>Master&apos;s thesis · Grade 4</span><strong>Software Developers and Generative AI: Job Demands and Resources</strong><em>Check thesis →</em></a></div></div></article><button className="courses-toggle" onClick={() => setCoursesOpen(!coursesOpen)} aria-expanded={coursesOpen}>Selected university courses <span>37</span> <b>{coursesOpen ? '−' : '+'}</b></button>{coursesOpen && <div className="courses-grid">{courses.map((course) => <span key={course}>{course}</span>)}</div>}</section>
        <section className="resume-section" id="projects"><h2>Selected projects</h2><p className="section-intro">A few recent projects - swipe or use the arrows to browse.</p><div className="project-grid"><article><video className="project-media" autoPlay muted loop playsInline><source src={media('Keittiövisu.mp4')} type="video/mp4" /></video><h3>Kitchen Renovation Visualizer</h3><p>Browser-based sales tool built with JavaScript, n8n workflows, and AI APIs for image generation and automation.</p></article><article><img className="project-media" src={media('Shift planning.png')} alt="Shift planning application dashboard" /><h3>Custom shift planning application in a few days</h3><p>Built a functioning app for gym workforce management using AI-assisted development.</p></article><article><div className="project-media-grid"><img src={media('wordpress1.png')} alt="Website example" /><img src={media('wordpress2.png')} alt="Website example" /></div><h3>High ranking websites</h3><p>WordPress websites built and optimized for search results and user experience.</p></article><article><img className="project-media" src={media('content.png')} alt="Media content creation example" /><h3>Media content creation</h3><p>Social media content creation and account management for business clients.</p></article></div></section>
        <section className="resume-section" id="creative"><h2>Bonus: Content creation</h2><p className="section-intro">A glimpse at the photography, editing and visual content I create.</p><div className="creative-grid"><article><h3>Video editing</h3><p>Filming and editing video content for digital channels and client content.</p><video ref={videoRef} className={videoIndex === 2 ? 'creative-video landscape' : 'creative-video'} autoPlay muted playsInline onEnded={() => setVideoIndex((index) => (index + 1) % videos.length)}><source src={media(videos[videoIndex])} type="video/mp4" /></video><div className="dots">{videos.map((name, index) => <button className={index === videoIndex ? 'active' : ''} key={name} onClick={() => setVideoIndex(index)} aria-label={name} />)}</div></article><article><h3>Still images and editing</h3><p>Still images and edited visual work from personal and creative projects.</p><div className="still-stage">{showingProfiles ? <div className="profile-bubbles">{profiles.map((name) => <img key={name} src={media(name)} alt={name} />)}</div> : <img src={media(stills[stillIndex])} alt={showingProfiles ? 'Profile photos' : `Photo ${stillIndex + 1}`} />}</div><div className="dots">{Array.from({ length: stillCount }, (_, index) => <button className={index === stillIndex ? 'active' : ''} key={index} onClick={() => setStillIndex(index)} aria-label={index === stills.length ? 'Profile photos' : `Photo ${index + 1}`} />)}</div></article></div></section>
        <section className="resume-section contact" id="contact"><h2>Get in touch</h2><p>I&apos;m currently open to new opportunities in digital services, data and development.</p><a className="contact-email" href="mailto:jasper.a.anttila@gmail.com">jasper.a.anttila@gmail.com</a><p>+358 40 567 2193 · LinkedIn · Jyväskylä, Finland</p></section>
